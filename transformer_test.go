// Test file for transformer attention mechanisms
package main
...
func RunTest(name string, test TestInput, showDetails bool) {
	fmt.Printf("\n" + "="*60 + "\n")
	fmt.Printf("TEST: %s\n", name)
	fmt.Printf("="*60 + "\n")
	...
	if showDetails {
		Q.Print("Query Matrix (Q)")
		K.Print("Key Matrix (K)")
		V.Print("Value Matrix (V)")
	}

	// Test single-head attention
	fmt.Println("--- Single-Head Attention ---")
	result1 := ScaledDotProductAttention(Q, K, V)
	
	if !showDetails {
		result1.Print("Final Attention Output")
	}

	// Test multi-head attention
	fmt.Println("\n--- Multi-Head Attention (2 heads) ---")
	mha := NewMultiHeadAttention(2, 8)
	result2 := mha.Forward(Q, K, V)
	
	if !showDetails {
		result2.Print("Multi-Head Attention Output")
	}
...
func TestIdentityMatrices() {
	...
	result := ScaledDotProductAttention(Q, K, V)
...
func TestRandomMatrices() {
	...
	Q.Print("Random Query Matrix")
	K.Print("Random Key Matrix")
	V.Print("Random Value Matrix")
